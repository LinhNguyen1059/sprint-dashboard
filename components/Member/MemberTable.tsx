...
const columns: ColumnDef<CombinedIssue>[] = [
  {
    id: "id",
    header: () => null,
    cell: ({ row }) => (
...
export function MemberTable({ data: initialData }: { data: CombinedIssue[] }) {
  ...
  const trackerOptions = React.useMemo(() => {
    const trackers = initialData.map((issue) => issue.tracker);
    return Array.from(new Set(trackers)).filter(Boolean);
  }, [initialData]);
  const statusOptions = React.useMemo(() => {
    const statuses = initialData.map((issue) => issue.status);
    return Array.from(new Set(statuses)).filter(Boolean);
  }, [initialData]);
  const priorityOptions = React.useMemo(() => {
    const priorities = initialData.map((issue) => issue.priority);
    ...
    <div className="w-full flex-col justify-start gap-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger size="sm" className="w-32" id="priority-filter">
